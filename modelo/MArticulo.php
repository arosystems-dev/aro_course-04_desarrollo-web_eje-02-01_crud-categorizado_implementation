   ...
   class MArticulo
   {
      ...
      public static function all($criterio){         
         ...
         $consulta = $conexion->prepare('
            SELECT a.idArticulo as idArticulo, a.codigo as codigo, a.nombre as nombre, c.nombre as categoria, a.foto as foto, a.stock as stock, a.estado as estado
            FROM articulo AS a INNER JOIN categoria AS c  ON c.idCategoria=a.idCategoria
            WHERE (a.estado=1) AND (a.codigo LIKE "%'.$criterio.'%" OR a.nombre LIKE "%'.$criterio.'%" OR c.nombre LIKE "%'.$criterio.'%") ORDER BY a.idArticulo DESC
         ');            
         $consulta->execute();         
      ...
      public function update($id){
         $conexion = ConexionPDO::openConexion();         
         $consulta = $conexion->prepare('UPDATE articulo SET codigo = ?, nombre = ?, idCategoria = ?, stock = ?, foto = ?, estado = ? WHERE idArticulo = ?');
